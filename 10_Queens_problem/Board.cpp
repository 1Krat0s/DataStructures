...
void Board::place_queens_all() 
{
    int count = 0;
    place_queens_all_helper(0, count);
    std::cout << "Total solutions found: " << count << std::endl;
}

void Board::place_queens_all_helper(int row, int& count) 
{
    if (row == board_size) 
    {
        count++;
    ...
    for (int col = 0; col < board_size; col++) 
    {
        if (check_place(row, col)) 
        {
            ...
            place_queens_all_helper(row + 1, count);
